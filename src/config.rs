...
impl Config {
    pub fn from_args(args: Args) -> Result<Self> {
        ...
        config.process_name = args.process;
        ...
        // Parse key sequence or single key
        if let Some(sequence_str) = args.sequence {
        ...
        if config.key_sequence.is_empty() {
            anyhow::bail!("No key sequence specified. Use --key for single key or --sequence for multiple keys.");
        }
